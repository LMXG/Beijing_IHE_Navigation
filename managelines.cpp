    ...
    ui(new Ui::ManageLines)
{
    ...
    tabWigetsVector.push_back(ui->tabAddSpot);
    tabWigetsVector.push_back(ui->tabAddConnection);
    tabWigetsVector.push_back(ui->tabAddByText);

    tabIconVector.push_back(QIcon(QPixmap(":/icon/icon/graden.png")));
    tabIconVector.push_back(QIcon(QPixmap(":/icon/icon/spot.png")));
    tabIconVector.push_back(QIcon(QPixmap(":/icon/icon/connect.png")));
    tabIconVector.push_back(QIcon(QPixmap(":/icon/icon/text.png")));

    tabTextVector.push_back("路线(&L)");
    tabTextVector.push_back("景点(&S)");
...
//设置添加景点部件可见
void ManageLines::setAddSpotVisible()
{
    ui->tabWidget->clear();
    ui->tabWidget->addTab(tabWigetsVector[1],tabIconVector[1],tabTextVector[1]);
}
...
{
    ui->comboBoxConnectSpot1->clear();
    ui->comboBoxConnectSpot2->clear();
    ui->comboBoxConnectLine->clear();

    for (auto &a: spotsNameList)
    {
        ui->comboBoxConnectSpot1->addItem(a);
        ui->comboBoxConnectSpot2->addItem(a);
    }
...
{
    lineName=arg1;
    ui->textBrowserAddLine->setText(tr("路线：")+lineName+"\n"
...
//景点编辑内容改变
void ManageLines::on_lineEditSpotName_textChanged(const QString &arg1)
{
    spotName=arg1;
    ui->textBrowserAddSpot->setText(tr("站名：")+spotName+"\n"+
